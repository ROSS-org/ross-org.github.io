...
One factor that can play a role is how the LP are mapped to the physical hardware underneath.
LPs that communicate frequently may benefit from being placed within the same MPI process, while LPs that communicate infrequently are placed on different MPI processes.
...
- **Physical Process or PE:**
  ...
  There are also several important global variable and configuration parameters that user should know about:
...
Finally, there are some global variables that may be helpful:
...
The implementation of these mapping function can be found in `ROSS/core/tw-setup.c`.
...
There are three mapping functions which must have the following function signatures:
...
The mapping setup function quite complex.
